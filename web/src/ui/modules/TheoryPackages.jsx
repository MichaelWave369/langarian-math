...
function PackageCard({ item, active, onSelect }) {
  ...
  return (
    <button type="button" className={`theory-card${active ? ' theory-card-active' : ''}`} onClick={onSelect}>
      <span className="theory-card-top">
        <strong>{item.theory.name}</strong>
        <PackageBadge tone={executable ? 'pass' : 'neutral'}>L{item.maturity_level}</PackageBadge>
      </span>
      <span className="dim-text">{packageLevelName(item.maturity_level)}</span>
      <span>{item.theory.summary}</span>
      <span className="theory-card-foot">
        <PackageBadge>{item.theory.status}</PackageBadge>
        <PackageBadge tone={executable ? 'pass' : 'warn'}>{executable ? 'executable' : 'not executable yet'}</PackageBadge>
      </span>
    </button>
  )
}
  ...
  return (
    <section className="panel panel-formal" aria-labelledby="manifest-preview-heading">
      <div className="package-title-row">
        <h2 id="manifest-preview-heading">Generated package manifest</h2>
        <PackageBadge tone={validation.ok ? 'pass' : 'fail'}>{validation.ok ? 'VALID' : `${validation.issues.length} ISSUE(S)`}</PackageBadge>
      </div>
      {!validation.ok && (
...
export default function TheoryPackages() {
  ...
  const receiptTemplate = useMemo(() => {
    ...
    return buildReceiptEnvelope(active, active.operators[0].id, {
      timestamp_utc: 'RUNTIME_TIMESTAMP',
      claims_supported: active.claim_boundaries.allowed.slice(0, 1),
      status: 'NOT_RUN',
    })
  }, [active])
  ...
  const useActiveAsTemplate = () => {
    ...
    setDraft(null)
    setModule('theories')
    setNotice('The selected package was copied into the wizard as a non-authoritative candidate fork.')
  }
  ...
  const importPackage = () => {
    ...
    setPackages((current) => {
      const withoutExisting = current.filter((item) => item.theory.id !== parsed.package.theory.id)
      return [...withoutExisting, parsed.package]
    })
  ...
  const exportPackage = (item) => {
    downloadText(`${sanitizeFilename(item.theory.id)}.theory-package.json`, `${JSON.stringify(item, null, 2)}\n`)
  }
  ...
  return (
    <div>
      <section className="theory-hero" aria-labelledby="theory-front-door-heading">
        <div>
          <PackageBadge tone="pass">THEORY PACKAGE ARCHITECTURE v0.1</PackageBadge>
          <h2 id="theory-front-door-heading">Where a theory enters Parallax</h2>
          <p>
            A theory does not get pasted into the finite-vector kernel and declared valid. It enters as a package that
            names its objects, operations, assumptions, invariants, implementations, receipts, and claim boundaries.
          </p>
          <p className="dim-text">
            The governance shell is general. Execution is package-specific and must be earned through implementation and conformance.
          </p>
        </div>
        <div className="hero-answer">
          ...
          <p>Only packages with lawful executable operators reach Level 3. Only independently checked packages reach Level 4.</p>
      ...
      <section className="panel panel-formal" aria-labelledby="maturity-heading">
        ...
        <div className="maturity-grid">
          {MATURITY_LEVELS.map((level) => (
            <article key={level.level} className="maturity-card">
              <PackageBadge>L{level.level}</PackageBadge>
              <h3>{level.name}</h3>
      ...
      <div className="two-col theory-two-col">
        <section className="panel panel-formal" aria-labelledby="package-library-heading">
          <div className="package-title-row">
            <h2 id="package-library-heading">Theory package library</h2>
            <PackageBadge>{packages.length} package(s)</PackageBadge>
          </div>
          <div className="theory-card-grid">
            {packages.map((item) => (
              <PackageCard
                key={`${item.theory.id}:${item.theory.version}`}
                item={item}
                active={active.theory.id === item.theory.id}
                onSelect={() => setActiveId(item.theory.id)}
              />
            ))}
          </div>
        </section>
        ...
        <section className="panel panel-formal" aria-labelledby="active-package-heading">
          <div className="package-title-row">
            <div>
              <h2 id="active-package-heading">{active.theory.name}</h2>
              <p className="mono dim-text">{active.theory.id}@{active.theory.version}</p>
            </div>
            <PackageBadge tone={activeValidation.ok ? 'pass' : 'fail'}>{activeValidation.ok ? 'MANIFEST VALID' : 'INVALID'}</PackageBadge>
          </div>
          ...
          <div className="btn-row" style={{ marginTop: 12 }}>
            {canExecutePackage(active) && active.metadata.execution_route === 'program' && (
              <button type="button" className="btn btn-primary" onClick={() => setModule('program')}>Open executable workbench</button>
            )}
          ...
          {!canExecutePackage(active) && (
            <div className="notice package-inline-notice" role="status">
              This package can be inspected now, but it has not earned executable status. Its open semantics stay visible rather than being simulated by the wrong kernel.
            </div>
          )}
        </section>
      </div>
      ...
      <div className="two-col theory-two-col">
        <section className="panel panel-formal" aria-labelledby="definitions-heading">
          ...
          <DefinitionList title="Operators" items={active.operators} />
        ...
        <section className="panel panel-formal" aria-labelledby="boundary-heading">
          ...
          <div className="claim-columns">
            <div>
              <h3>Allowed</h3>
              <ul>{active.claim_boundaries.allowed.map((claim) => <li key={claim}>{claim}</li>)}</ul>
            </div>
            <div>
              <h3>Prohibited</h3>
              <ul>{active.claim_boundaries.prohibited.map((claim) => <li key={claim}>{claim}</li>)}</ul>
            </div>
          </div>
          <p className="panel-sub">
            The generic envelope binds a future execution to its package, operator, implementation, assumptions, checks,
            parents, and bounded claims. It does not claim the operation ran until status and outputs are populated.
          </p>
      ...
      <section className="panel panel-formal" aria-labelledby="wizard-heading">
        <div className="package-title-row">
          <div>
            <h2 id="wizard-heading">Theory Definition Wizard</h2>
            <p className="panel-sub">Create a documentary package first. Open definitions remain marked THEORY MAP OPEN rather than being invented.</p>
          </div>
          <button type="button" className="btn btn-ghost" onClick={() => { setForm(EMPTY_FORM); setDraft(null) }}>Clear</button>
        </div>
        <div className="theory-form-grid">
          ...
          <label className="field"><span>maturity level</span>
            <select value={form.maturityLevel} onChange={(event) => updateForm('maturityLevel', Number(event.target.value))}>
              {MATURITY_LEVELS.map((item) => <option key={item.level} value={item.level}>L{item.level} — {item.name}</option>)}
            </select>
          </label>
        ...
        <div className="btn-row" style={{ marginTop: 12 }}>
          <button type="button" className="btn btn-primary" onClick={buildDraft}>Build &amp; validate manifest</button>
          {draft && <button type="button" className="btn" onClick={() => exportPackage(draft)}>Export draft</button>}
        </div>
      </section>
      ...
      <section className="panel panel-formal" aria-labelledby="import-package-heading">
        ...
        <p className="panel-sub">Import is local to this browser session. Schema validation happens before the package enters the library.</p>
        <textarea
          className="code"
          rows={8}
          value={importText}
          onChange={(event) => setImportText(event.target.value)}
          placeholder='{"schema_version":"theory-package:v0.1", ...}'
          aria-label="theory package JSON"
        />
        <div className="btn-row" style={{ marginTop: 10 }}>
          <button type="button" className="btn btn-primary" onClick={importPackage} disabled={importText.trim() === ''}>Validate &amp; import</button>
        </div>
        {importValidation && (
          <div className={importValidation.ok ? 'package-validation-pass' : 'error-box'} role="status">
            {importValidation.ok ? 'Package manifest is valid and was imported into this session.' : (
              <ul className="issue-list">
                {importValidation.issues.map((issue, index) => <li key={`${issue.path}:${index}`}><code>{issue.path}</code> [{issue.code}] {issue.message}</li>)}
              </ul>
            )}
          </div>
        )}
