...
class BoundarySample:
    """One finite sample for a reduced boundary-domain screen."""
    ...
    def __post_init__(self) -> None:
        ...
        _finite_float(self.curvature, "curvature")

    @property
    def boundary_value(self) -> float:
        return boundary_value(self.kappa, self.c, self.curvature)
    ...
    def to_dict(self) -> dict[str, float]:
        return {
            ...
            "B_t": self.boundary_value,
        }

    @classmethod
    def from_mapping(cls, data: Mapping[str, Any]) -> "BoundarySample":
        return cls(
            ...
            curvature=_finite_float(
                _lookup(data, "curvature", "v_gamma_gamma", "V_gamma_gamma", "Vgg"),
                "curvature",
            ),
        )
...
@dataclass(frozen=True)
class BoundaryScan:
    """Receipt-bearing result of a finite boundary-domain scan."""

    samples: tuple[BoundarySample, ...]
    min_boundary_value: float
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "min_boundary_value": self.min_boundary_value,
...
def boundary_value(kappa: float, c: float, curvature: float) -> float:
    """Compute a scalar reduced-domain boundary value for supplied finite inputs."""

    kappa_f = _finite_float(kappa, "kappa")
    c_f = _finite_float(c, "c")
    curvature_f = _finite_float(curvature, "curvature")
    return float(1.0 - 6.0 * (kappa_f**2) * (c_f**2) * curvature_f)
...
def _domain_invariant(values: tuple[float, ...], violations: tuple[int, ...], *, strict: bool) -> InvariantResult:
    ...
    if not strict and any(math.isclose(value, 0.0, abs_tol=1e-12) for value in values):
        return InvariantResult(
            "D3.boundary_domain",
            ResultStatus.WARN,
            "At least one sample touches the boundary B(t)=0.",
...
def scan_boundary_domain(
    samples: Sequence[BoundarySample | Mapping[str, Any]],
    *,
    strict: bool = True,
    label: str | None = None,
) -> BoundaryScan:
    """Scan finite samples for a reduced boundary-domain rule.
    ...
    parsed = tuple(sample if isinstance(sample, BoundarySample) else BoundarySample.from_mapping(sample) for sample in samples)
    if not parsed:
        raise ValueError("scan_boundary_domain requires at least one sample.")

    values = tuple(sample.boundary_value for sample in parsed)
    if strict:
    ...
    output_payload = {
        "operator": "boundary_domain_scan",
        "strict": bool(strict),
        "label": label,
        "sample_count": len(parsed),
        "min_boundary_value": min(values),
    ...
    receipt = OperationReceipt(
        ...
        claims=[
            Claim(
                "Boundary-domain values were computed from supplied finite samples as a reduced-domain screen, not as proof of a larger model.",
                EpistemicTag.MODEL,
            )
        ],
    )
    ...
    return BoundaryScan(
        samples=parsed,
        min_boundary_value=float(min(values)),
        violating_indices=violations,
        receipt=receipt,
    )


BracketSample = BoundarySample
BracketWallScan = BoundaryScan
bracket_wall_value = boundary_value
scan_bracket_wall = scan_boundary_domain
